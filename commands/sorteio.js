...
exports.run = async (bot, message, args) => { 
  ...
  .setColor('#8c0046')   
    ...
    if (!channel)
      return message.channel.send(
        `Canal não encontrado por favor use: sorteio tempo #canal premio`
      );
    let prize = args.slice(2).join(" ");
    if (!prize) return message.channel.send(`Premio não especificado!`);
    message.channel.send(`*Sorteio iniciado em ${channel}*`);
    ...
    setTimeout(() => {
      if (m.reactions.get("🎉").count <= 1) {
        message.channel.send(`Reações: ${m.reactions.get("🎉").count}`);
        return message.channel.send(
